...
- 사용자의 잔액 정보를 저장하는 테이블
- 사용자 잔액은 수정이 빈번하기 때문에, 별도의 테이블로 분리하여 관리
...
# COUPON_MOLD_INVENTORY (쿠폰 모형 재고)

| 필드명              | 타입      | 설명      |
|------------------|---------|---------|
| id               | BIGINT  | PK      |
| stock            | INT     | 재고      |
| coupon_mold_id   | BIGINT  | FK      |
| created_at       | DATE    | 생성일     |
| last_modified_at | DATE    | 마지막 수정일 |

- 쿠폰 모형의 재고 정보를 저장하는 테이블
- 쿠폰 모형의 재고는 수정이 빈번하기 때문에, 별도의 테이블로 분리하여 관리
...
- `user_coupon_id` 필드는 주문 결제 시 사용된 쿠폰의 ID
...
- 주문한 상품 정보를 저장하는 테이블
- 상품 주문 시점에 가격 및 할인 정보 등을 저장하는 스냅샷
